...
abstract class Status(var remainingTime: Int) {
  ...
  def applyEffect(result: StatusResults): Unit = {
    if (remainingTime >= 0)
      remainingTime -= 1
  }
}
...
class BleedingStatus(remainingTime: Int) extends Status(remainingTime) {
  ...
  override def applyEffect(result: StatusResults): Unit = {
    result.healthModifier -= 3
    super.applyEffect(result)
  }
}
